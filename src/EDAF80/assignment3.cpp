...
edaf80::Assignment3::Assignment3(WindowManager &windowManager)
  ...
  window = mWindowManager.CreateGLFWWindow("EDAF80: Assignment 3", window_datum,
                                           config::msaa_rate);
  if (window == nullptr) {
...
edaf80::Assignment3::~Assignment3() { bonobo::deinit(); }
...
void edaf80::Assignment3::run() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (opened) {
      ...
      auto demo_sphere_selection_result = program_manager.SelectProgram(
          "Demo sphere", demo_sphere_program_index);
      if (demo_sphere_selection_result.was_selection_changed) {
        demo_sphere.set_program(demo_sphere_selection_result.program,
                                phong_set_uniforms);
      }
      ImGui::Separator();
      ...
      ImGui::SliderFloat3("Light Position", glm::value_ptr(light_position),
                          -20.0f, 20.0f);
      ImGui::Separator();
    ...
    if (opened)
      ImGui::Text(
          "%.3f ms",
          std::chrono::duration<float, std::milli>(deltaTimeUs).count());
    ImGui::End();
